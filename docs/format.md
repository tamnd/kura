...
The distinction matters for a memory mapped file.
Verifying the checksum means reading every page, which for a large segment is the difference between opening in microseconds and opening in the time it takes to fault in the whole thing.
